...
import fr.isen.ElGuerzyly.androiderestaurant.adapter.BasketAdapter
import fr.isen.ElGuerzyly.androiderestaurant.databinding.ActivityBasketBinding
...
class BasketActivity : MenuActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (file.exists()) {
            val dishesBasket : List<DishBasket> = Gson().fromJson(file.readText(), BasketList::class.java).data
    ...
    private fun display(dishesList : List<DishBasket>) {
        ...
        binding.basketList.adapter = BasketAdapter(dishesList) {deleteDishBasket(it)}
    }
